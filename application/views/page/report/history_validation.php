            ...
            <div class="row" style="margin-bottom:10px;">
                ...
                <div class="col-sm-2">
                    <select name="column" id="column" require class="form-control">
                        ...
                        <option value="prize">hadiah</option>
                    </select>
                </div>
                <div class="col-sm-2">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                    <div class="box-body">
                        ...
                        <div class="form-group row">
                            <label for="inputPassword3" class="col-sm-4 control-label">No Undian</label>
                            <div class="col-sm-8">
                        ...
                        <div class="form-group row">
                            <label for="inputPassword3" class="col-sm-4 control-label">Tanggal Validasi</label>
                            <div class="col-sm-8">
                                <p id="validationDate"></p>
                            </div>
                        </div>
...
<div class="modal fade" id="download" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title" id="exampleModalLabel">Download History Validation</h4>
            </div>
            <div class="modal-body">
                    <div class="box-body">
    ...
    $(document).ready(function() {
      ...
      $("#to2").val(getCurrentDate());
        ...
        var table = $('#table').DataTable({
            ...
            "paging":true,
            "ajax": {
                "url": "<?php echo base_url('ajax/Report_c/getDataHistoryValidation'); ?>",
                'data': function(data) {
                    // Read values
            ...
            "columns": [
                {
                    data: 'statusValidation',orderable: false
                },{data: 'name',orderable: false},{data: 'msisdn',orderable: false},{data: 'prize',orderable: false},{data: 'lotteryNumber',orderable: false},
                {
                    data: 'dateValidation',orderable: false
                },
                {
                    data: 'statusValidation',orderable: false
                },
            ...
            "columnDefs": [{
                ...
                "render": function (data, type, row, meta) {
                    var btn = '<a data-id="' + row.id + '" class="btn btn-xs btn-info" data-toggle="modal" data-target="#detail">Detail</a> ';
                    return btn;
                }
            }],
        ...
        $("#btn_download").click(function() {
            ...
            $.ajax({
                url: "<?php echo base_url('Job_c/create/download_history_validation'); ?>", // Ensure this resolves correctly
                method: "POST",
        ...
        function getDetailRedeem(id){
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    $("#lotteryNumber").text(response.lotteryNumber);
                    $("#redeemDate").text(response.dateRedeem);
                    $("#amount").text(response.amount);
                    $("#notes").text(response.notes);
                    $("#validationDate").text(response.dateValidation);
                    $("#status").text(response.approved);
                },
                error: function(xhr, status, error) {
                    alert("Failed to submit request. Please try again.");
                }
